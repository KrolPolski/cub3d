...
/*   Updated: 2024/05/28 17:29:56 by rboudwin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*void	get_len(t_map *map, double *x_len, double *y_len, double deg)
{
	...
	if (map->p_orient / DEG_2_RAD + deg == 90 || map->p_orient / DEG_2_RAD + deg == 270)
		...
		(*y_len) = floor(64 - (map->ray->ray_y - (64 *(floor(map->ray->ray_y / 64)))) / cos(corner));
	//	(*y_len) = (64 - ((map->ray->ray_y) % 64)) / cos(corner);
	else
	//	(*y_len) = ((map->ray->ray_y) % 64) / cos(corner);
		(*y_len) = 	floor(map->ray->ray_y - (64 *(floor(map->ray->ray_y / 64))) / cos(corner));
	if (corner > 0 && corner / DEG_2_RAD > 90)
  	...
  	if ((map->p_orient / DEG_2_RAD) + deg == 90 || (map->p_orient / DEG_2_RAD + deg == 270))
		...
		//{(*x_len) = (int)(((int)map->ray->ray_x) % 64) / cos(corner);
		//printf("x_len from old way: %f\n", *x_len);
		(*x_len) = floor(map->ray->ray_x - (64 *(floor(map->ray->ray_x / 64))) / cos(corner));
		//printf("x_len from new way: %f\n", *x_len);}
	else
		//(*x_len) = (64 - ((map->ray->ray_x) % 64)) / cos(corner);
		(*x_len) = floor(64 - (64 *(map->ray->ray_x - (floor(map->ray->ray_x) / 64))) / cos(corner));
	if (*y_len < 0)
        (*y_len) *= -1;
  	if (*x_len < 0)
        (*x_len) *= -1;
...
/* 	 	while (dist < map->rend_dist && dist <= y_len && dist <= x_len)
		...
		//dist++;
		//printf ("DEG %f dist = %f\n", deg, map->ray->dist);
	//	map->ray->ray_y = (((double)map->p_pos_y - (map->ray->dist + 1)  * cos(map->p_orient + (deg * DEG_2_RAD))));
	//	map->ray->ray_x = (((double)map->p_pos_x + (map->ray->dist + 1)  * sin(map->p_orient + (deg * DEG_2_RAD))));
