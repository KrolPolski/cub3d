...
void	cast_wall(t_map *map, double dist, float deg, enum e_dir dir, int *row)
{
	...
	pixels = 0;
/*  	if (deg < 0)
		angle = ((deg * (double)-1) + (double)90) * DEG_2_RAD;
	else
		angle = (deg + (double)90) * DEG_2_RAD; */
...
/* 	 	while (dist < map->rend_dist && dist <= y_len && dist <= x_len)
		...
		//printf("the ray is at = [%d][%d]\n", map->ray->ray_y % 64, map->ray->ray_x % 64);
...
/* 			if (map->ray->ray_y < (map->p_pos_y - 32 - (dist -1)  * cos(map->p_orient + (deg * DEG_2_RAD))))
		...
		{
			if (map->map_visible)
				mlx_put_pixel(images->fg, map->rend_dist +map->ray->dist * sin(map->p_orient + deg * DEG_2_RAD), map->rend_dist - map->ray->dist * cos(map->p_orient + deg * DEG_2_RAD), get_rgba(255, 0, 0, 255));
